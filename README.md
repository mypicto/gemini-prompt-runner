...
* URLのパラメータからプロンプトを自動実行（パラメータはサーバーに送信されずセキュアに処理されます）
  * クリップボードのテキストをプロンプトに挿入
  * オプションでプロンプトで自動送信を無効化することも可能
* URLのパラメータからモデルの選択
...
| パラメータ | 説明 | 値 |
| --- | --- | --- |
| `ext-q` | 実行するプロンプト文字列 | URLエンコードされたテキスト</br>`{{clipboard}}`キーワードでクリップボードのテキストを挿入 |
